	...
	for player_id in PLAYER_IDS:
		...
		if previous_location != location:
			...
			if player_id == _active_player_id:
				_show_area_card(location, false)
...
func _set_active_player(player_id: String) -> void:
	...
	_update_world_hud()
	_show_area_card(GameState.get_player_location(_active_player_id), true)
...
func _update_world_hud() -> void:
	...
	_active_player_value.text = "Focus: %s" % active_name
	_active_location_value.text = "Tab or Q swaps players."
	_interaction_hint_value.text = prompt_text if prompt_text != "" else "Move with WASD or arrows. Press E near a highlighted route."
...
func _location_presence_color(location_id: String, active_location: String, other_location: String, together: bool) -> Color:
	...
	if together and location_id == other_location:
		return Color(0.96, 0.98, 1.0, 0.88)

	if location_id == other_location:
		return Color(0.6, 0.66, 0.74, 0.48)

	if _is_related_location(active_location, location_id):
		return Color(0.4, 0.45, 0.5, 0.28)

	return Color(0.16, 0.18, 0.22, 0.12)
...
func _pawn_presence_color(player_id: String, active_location: String, together: bool) -> Color:
	...
	if together and location == active_location:
		return Color(0.88, 0.92, 0.96, 0.88)

	if location == active_location:
		return Color(0.76, 0.82, 0.9, 0.74)

	return Color(0.34, 0.38, 0.44, 0.18)
...
func _show_area_card(location: String, use_short_hold: bool) -> void:
	if not LOCATION_LABELS.has(location):
		return

	var subtitle: String = ""
	if AREA_CARD_SUBTITLES.has(location):
		subtitle = str(AREA_CARD_SUBTITLES[location])

	_area_card_title.text = str(LOCATION_LABELS[location]).to_upper()
	_area_card_subtitle.text = subtitle
	_area_card.visible = true
	_area_card.modulate = Color(1.0, 1.0, 1.0, 0.0)
	_area_card.scale = Vector2(0.98, 0.98)

	if _area_card_tween != null:
		_area_card_tween.kill()

	var hold_time: float = 1.15 if use_short_hold else 1.45
	_area_card_tween = create_tween()
	_area_card_tween.set_parallel(true)
	_area_card_tween.tween_property(_area_card, "modulate:a", 1.0, 0.18)
	_area_card_tween.tween_property(_area_card, "scale", Vector2.ONE, 0.18)
	_area_card_tween.chain().tween_interval(hold_time)
	_area_card_tween.chain().set_parallel(true)
	_area_card_tween.tween_property(_area_card, "modulate:a", 0.0, 0.28)
	_area_card_tween.tween_property(_area_card, "scale", Vector2(1.0, 1.02), 0.28)
	_area_card_tween.chain().tween_callback(_hide_area_card)


func _hide_area_card() -> void:
...
func _toggle_debug_panel() -> void:
	...
	_debug_toggle_button.text = "Hide Debug" if _debug_panel.visible else "Debug"
